<div class="row" >
	<div class="col-md-3 col-ms-3 col-mx-3"><p class="button" id="b1"><a href="index.php?enregistrement"><img src="view/image/en1.png" width="30%"><br/>ENREGISTRER UTILISATEUR</a></p></div>

	<div class="col-md-5" id="tab">
		<h3>LISTE VISITEUR DANS LE BATIMENT</h3>
		<div id="liste" class="vizi">

		</div>
		<div><a class="btn" id="btn2" href="index.php?verification" >Liste-Complete</a></div>
	</div>

	<div class="col-md-3"><p class="button" id="b3"><a href="index.php?rapport"><img src="view/image/rap1.png" width="30%"><br/>RAPPORT</p></div>

	<div class="col-md-3"><p class="button" id="b4"><a href="index.php?recherche"><img src="view/image/re1.png" width="30%"><br/>RECHERCHER</p></div>
</div>
<script src="view/js/listevisiteur.js"></script>
